...
class BloomierFilterImmutable(BloomierBase):
    def __init__(self, size: int, num_hashes: int, val_max_bit_length: int, seed: int = 0):
        ...
        self.__val_max_bit_length = val_max_bit_length
    ...
    def construct(self, input_dict: dict):
        ...
        for key, tweak, neighbors in ordered_key_neighbors:
            ...
            for neighbor in neighbors:
                val ^= self.__table1[neighbor]
            self.__table1[tweak] = val

    def get(self, key):
        result = self._get_mask(key)
        for neighbor in self._hash(key):
            result ^= self.__table1[neighbor]
        if result.bit_length() > self.__val_max_bit_length:
            return None
        return result
    ...
    def __validate(self, input_dict: dict):
        ...
        for key, val in input_dict.items():
            ...
            if val.bit_length() > self.__val_max_bit_length:
                raise Exception(f'Value {val} should be smaller than {1 << self.__val_max_bit_length}.')
